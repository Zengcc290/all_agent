...
def apply_embedding_lock(
    ...
    *,
    ...
    """Gate a vector write against the SQLite lock.

    ``:memory:`` / missing repository is a no-op so unit tests keep using
    isolated in-memory stores. An empty lock is claimed by the current
    embedding. A mismatch raises unless ``confirm_rebuild`` is set, in which
    case the vector collection is recreated and SQLite truth is reindexed.
    """
    ...
    if locked.model == current.model and locked.dimension == current.dimension:
        return locked
    if not confirm_rebuild:
        raise EmbeddingLockMismatch(locked, current)
