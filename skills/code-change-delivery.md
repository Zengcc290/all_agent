...
2. 用 `git status` 命令查看工作区：branch、staged / unstaged / untracked / conflicted，确认本次涉及哪些文件。
...
用 `git commit`，message 固定使用刚返回的 `update_id`，格式：
...
- 建议用 `git add <路径>` 显式列出本次涉及的文件，避免把无关的临时文件、密钥或 `.env` 带入提交；如确有其他未跟踪文件属于本任务，一并列出。
- 不得使用下一次编号或自行编造编号。

### 第 5 步：推送远端

`git push` 默认推送到当前上游；若分支无上游先 `git push -u` 设置。推送失败时读取推送输出诊断原因（无凭据/网络/无远端），处理后再推送；无法解决时如实向用户报告，在远端同步成功前不得声称任务已完成。
...
| push 失败 | 读推送输出：无凭据→配置凭据助手；无远端→检查 remote；无上游→用 -u |
| 工作区有无关未跟踪文件 | 用 `git add <路径>` 显式选择，不提交无关文件 |

## 与工具集的关系

本技能落地方式：用 `git status` / `git add` / `git commit` / `git push` 直接命令完成 git 操作，用 `fs.read_text` / `fs.read_dir` 核对文件，直接运行 pytest / ruff 完成验证；日志写入必须经 `system.update_log` 工具，不要手写 SQL 或直接改库。
